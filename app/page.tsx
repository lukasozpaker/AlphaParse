...
export default function FinancialSearch() {
  ...
  const generateAIResponse = (message: string, ticker: string): string => {
    ...
    } else if (message.toLowerCase().includes("profit") || message.toLowerCase().includes("income")) {
  ...
  return (
    <div className="container mx-auto py-10 px-4 pb-24">
      <h1 className="text-3xl font-bold text-center mb-8">Financial Data Search</h1>
        ...
        <Card className="h-[200px] flex flex-col">
          {tickerData ? (
            <>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  <span className="text-lg font-normal text-muted-foreground">{tickerData.companyName}</span>
                </CardTitle>
                <CardDescription>
              ...
              <CardContent className="flex-grow flex items-center">
                <div className="grid grid-cols-2 gap-4 w-full">
                  <div className="space-y-1">
                    ...
                    <p className="text-lg font-medium">{tickerData.marketCap}</p>
                  </div>
                  <div className="space-y-1">
            ...
            <div className="flex flex-col justify-center items-center h-full">
              ...
              <p className="text-center text-muted-foreground">Search for a ticker symbol to see company information</p>
            </div>
          )}
        </Card>
      </div>
              ...
              <Card className="mb-6 shadow-md">
                ...
                <CardContent>
                  <form onSubmit={handleInfoQuery} className="flex gap-2">
                    <Input
                      placeholder={
                        selectedTicker ? "E.g., current year revenue, profit margin" : "Search for a ticker first..."
                      }
                      value={query}
                ...
                <Card key={result.id} className="border-2 border-primary/20">
                  ...
                  <CardContent className="pt-6">
                    <div className="border rounded-lg p-4">
                      <p className="text-sm text-muted-foreground mb-2">Query: "{result.query}"</p>
                        ...
                        <div className="mt-2">
                          ...
                          <p
                            className={`font-medium ${
                              result.growth.startsWith("+") ? "text-green-600" : "text-red-600"
                            }`}
                          >
                            {result.growth}
                          </p>
                ...
                {selectedTicker ? (
                  chatMessages.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground">
                      ...
                      <p>No messages yet. Start chatting about {selectedTicker}!</p>
                    </div>
                  ) : (
                    <div className="space-y-4">
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground">
                    ...
                    <p>Please select a ticker symbol first to start chatting.</p>
                  </div>
                )}
              </div>
            </CardContent>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-background border-t shadow-lg z-10">
          <div className="container mx-auto px-4 py-4">
            <form onSubmit={handleChatSubmit} className="flex w-full gap-2">
              <Input
                placeholder={selectedTicker ? `Ask about ${selectedTicker}...` : "Search for a ticker first..."}
                value={chatInput}
